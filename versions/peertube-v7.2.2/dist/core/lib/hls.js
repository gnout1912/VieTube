...
function updateMasterHLSPlaylist(video, playlistArg) {
    return playlistFilesQueue.add(async () => {
        ...
        for (const caption of captions) {
            if (!caption.m3u8Filename)
                continue;
            extMediaSubtitle.push(`#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subtitles",` +
...
function updateSha256VODSegments(video, playlistArg) {
    return playlistFilesQueue.add(async () => {
        const json = {};
        const playlist = await VideoStreamingPlaylistModel.loadWithVideoAndFiles(playlistArg.id);
