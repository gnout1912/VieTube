...
function checkEmailConfig() {
    if (!isEmailEnabled()) {
        ...
        if (CONFIG.SIGNUP.ENABLED && CONFIG.SIGNUP.REQUIRES_APPROVAL) {
            logger.warn('SMTP is not configured but signup approval is enabled: ' +
...
function checkObjectStorageConfig() {
    ...
    if (CONFIG.OBJECT_STORAGE.MAX_UPLOAD_PART > parseBytes('250MB')) {
        logger.warn(`Object storage max upload part seems to have a big value (${CONFIG.OBJECT_STORAGE.MAX_UPLOAD_PART} bytes). ` +
