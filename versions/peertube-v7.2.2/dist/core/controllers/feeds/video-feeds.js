...
import { asyncMiddleware, commonVideosFiltersValidator, feedsAccountOrChannelFiltersValidator, feedsFormatValidator, setDefaultVideosSort, setFeedFormatContentType, videosSortValidator, videoSubscriptionFeedsValidator } from '../../middlewares/index.js';
import { buildFeedMetadata, getCommonVideoFeedAttributes, getPodcastFeedUrlCustomTag, getVideosForFeeds, initFeed, sendFeed } from './shared/index.js';
...
async function generateVideoFeed(req, res) {
    ...
    const data = await getVideosForFeeds(Object.assign(Object.assign({}, buildNSFWFilters({ req, res })), { sort: req.query.sort, isLocal: req.query.isLocal, include: req.query.include | VideoInclude.FILES, accountId: account === null || account === void 0 ? void 0 : account.id, videoChannelId: videoChannel === null || videoChannel === void 0 ? void 0 : videoChannel.id }));
...
async function generateVideoFeedForSubscriptions(req, res) {
    ...
    const data = await getVideosForFeeds(Object.assign(Object.assign({}, buildNSFWFilters({ req, res })), { sort: req.query.sort, isLocal: req.query.isLocal, include: req.query.include | VideoInclude.FILES, displayOnlyForFollower: {
...
function addVideosToFeed(feed, videos) {
    for (const video of videos) {
        ...
        feed.addItem(Object.assign(Object.assign({}, getCommonVideoFeedAttributes(video)), { id: WEBSERVER.URL + video.getWatchStaticPath(), author: [
            ...
            ], torrents, video: bestFiles.length !== 0
                ? {
